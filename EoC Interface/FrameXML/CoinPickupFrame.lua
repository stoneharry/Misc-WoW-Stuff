...
function OpenCoinPickupFrame(multiplier, maxMoney, parent)
	...
	if ( ENABLE_COLORBLIND_MODE == "1" ) then
		...
		elseif ( multiplier == COPPER_PER_PLATINUM ) then
			CoinPickupFrame.symbol = PLAT_AMOUNT_SYMBOL;
		end
	else
		...
		if ( multiplier == COPPER_PER_PLATINUM ) then
			CoinPickupPlatinumIcon:Show();
		else
			CoinPickupPlatinumIcon:Hide();
		end
	end
