...
function PVPBattleground_UpdateBattlegrounds()
	...
	for i=1,GetNumBattlegroundTypes() do
		...
		if ( (localizedName and canEnter) or (localizedName == "The Divide" ) ) then
			if ( frame ) then
				frame.BGindex = i;
...
function PVPBattleground_UpdateInfo(BGindex)
	...
	if BGname == "The Divide" then
		PVPBattlegroundFrameBGTex:SetTexture(PVPBATTLEGROUND_TEXTURELIST[32]);
	end
	...
	if ( isRandom or isHoliday ) then
		...
		if mapName == nil or mapName == "The Divide" then
			maxGroup = 1
			mapName = "The Divide"
			mapDescription = "The Divide is a subject of survival. You join with a group of other people in a jungle. Your objective? To survive as long as possible. The last person alive wins.\n\nKilling players gives you special rewards. More spells and items can be found be exploring.\n\nHow long can you survive?"
		end
