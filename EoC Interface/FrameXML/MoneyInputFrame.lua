...
function MoneyInputFrame_ResetMoney(moneyFrame)
	_G[moneyFrame:GetName().."Platinum"]:SetText("");
...
function MoneyInputFrame_ClearFocus(moneyFrame)
	_G[moneyFrame:GetName().."Platinum"]:ClearFocus();
...
function MoneyInputFrame_GetCopper(moneyFrame)
	...
	local platinum = _G[moneyFrame:GetName().."Platinum"]:GetText();
	...
	if ( platinum ~= "" ) then
		totalCopper = totalCopper + (platinum * COPPER_PER_PLATINUM);
	end
	return totalCopper;
end
...
function MoneyInputFrame_SetTextColor(moneyFrame, r, g, b)
	...
	_G[moneyFrame:GetName().."Platinum"]:SetTextColor(r, g, b);
end
...
function MoneyInputFrame_SetCopper(moneyFrame, money)
	...
	editbox = _G[moneyFrame:GetName().."Platinum"];
	if ( editbox:GetNumber() ~= platinum ) then
		editbox:SetNumber(platinum);
		moneyFrame.expectChanges = moneyFrame.expectChanges + 1;
	end
end
...
function MoneyInputFrame_SetMode(frame, mode)
	...
	if ( mode == "compact" ) then
		...
		_G[frameName.."Gold"]:SetPoint("LEFT", frameName.."Platinum", "RIGHT", 33, 0);
		_G[frameName.."Platinum"]:SetWidth(24);
	end
end
