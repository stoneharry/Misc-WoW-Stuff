...
function LootFrame_UpdateButton(index)
	...
	if ( slot <= numLootItems ) then	
		if ( (LootSlotIsItem(slot) or LootSlotIsCoin(slot)) and index <= numLootToShow ) then
			...
			local g = {};
			for a, b in string.gmatch(item, "(%d+) (%a+)") do
				table.insert(g, tonumber(a));
				table.insert(g, " "..b);
			end
			if (g[1] and g[2] == " Gold" and 100 < g[1]) then
				for i = 0, 5 do
					g[8-i] = g[6-i];
				end
				g[2] = " Platinum";
				g[1] = floor(g[3] / GOLD_PER_PLATINUM);
				g[3] = g[3] - g[1] * GOLD_PER_PLATINUM;
				if (g[3] == 0) then
					g[3] = g[5];
					g[4] = g[6];
					g[5] = g[7];
