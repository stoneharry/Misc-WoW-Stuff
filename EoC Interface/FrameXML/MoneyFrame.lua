...
COPPER_PER_PLATINUM = COPPER_PER_SILVER * SILVER_PER_GOLD * GOLD_PER_PLATINUM;
...
function MoneyFrame_SetType(self, type)
	...
	if ( info.canPickup ) then
		_G[frameName.."PlatinumButton"]:EnableMouse(true);
		...
		_G[frameName.."PlatinumButton"]:EnableMouse(false);
...
function MoneyFrame_Update(frameName, money)
	...
	local platinum = floor(money / COPPER_PER_PLATINUM);
	local gold = floor((money - (platinum * COPPER_PER_PLATINUM)) / COPPER_PER_GOLD);
	local silver = floor((money - (platinum * COPPER_PER_PLATINUM + gold * COPPER_PER_GOLD)) / COPPER_PER_SILVER);
	local copper = mod(money, COPPER_PER_SILVER);

	local platinumButton = _G[frameName.."PlatinumButton"];
	...
	if ( ENABLE_COLORBLIND_MODE == "1" ) then
		if ( not frame.colorblind or not frame.vadjust or frame.vadjust ~= MONEY_TEXT_VADJUST ) then
			...
			platinumButton:SetNormalTexture("");
			goldButton:SetNormalTexture("");
			silverButton:SetNormalTexture("");
			copperButton:SetNormalTexture("");
			_G[frameName.."PlatinumButtonText"]:SetPoint("RIGHT", 0, MONEY_TEXT_VADJUST);
		...
		platinumButton:SetText(platinum .. PLATINUM_AMOUNT_SYMBOL);
		platinumButton:SetWidth(goldButton:GetTextWidth());
		platinumButton:Show();
		...
		if ( frame.colorblind or not frame.vadjust or frame.vadjust ~= MONEY_TEXT_VADJUST ) then
			...
			frame.vadjust = MONEY_TEXT_VADJUST;
			local texture = CreateMoneyButtonNormalTexture(platinumButton, iconWidth);
			...
			_G[frameName.."PlatinumButtonText"]:SetPoint("RIGHT", -iconWidth, MONEY_TEXT_VADJUST);
		...
		platinumButton:SetText(platinum);
		platinumButton:SetWidth(platinumButton:GetTextWidth() + iconWidth);
		platinumButton:Show();
	...
	local showLowerDenominations, truncateCopper, truncateSilver;
	if ( platinum > 0 ) then
		width = width + platinumButton:GetWidth();
	...
	platinumButton:ClearAllPoints();
	if ( gold > 0 or showLowerDenominations ) then
		-- Exception if showLowerDenominations and fixedWidth
		if ( showLowerDenominations and info.fixedWidth ) then
			goldButton:SetWidth(COIN_BUTTON_WIDTH);
		end
		
		width = width + goldButton:GetWidth();
		platinumButton:SetPoint("RIGHT", frameName.."GoldButton", "LEFT", spacing, 0);
		if ( platinumButton:IsShown() ) then
			width = width - spacing;
		end
		if ( info.showSmallerCoins ) then
			showLowerDenominations = 1;
		...
		goldButton:Hide();
		platinumButton:SetPoint("RIGHT", frameName.."GoldButton",	"RIGHT", 0, 0);
	end
	
	goldButton:ClearAllPoints();
	if ( (silver > 0 or showLowerDenominations) and not truncateSilver) then
		-- Exception if showLowerDenominations and fixedWidth
		if ( showLowerDenominations and info.fixedWidth ) then
...
function SetMoneyFrameColor(frameName, color)
	...
	platinumButton:SetNormalFontObject(fontObject);
...
function GetDenominationsFromCopper(money)
	...
	local c = {}
	if (0 < platinum) then
		tinsert(c, tostring(platinum).." Platinum");
	end
	if (0 < gold) then
		tinsert(c, tostring(gold).." Gold");
	end
	if (0 < silver) then
		tinsert(c, tostring(silver).." Silver");
	end
	if (0 < copper) then
		tinsert(c, tostring(copper).." Copper");
	end
	return table.concat(c, " ");
	--return GetCoinText(money, " ");
end

GetCoinText = GetDenominationsFromCopper;

function GetMoneyString(money)
	local platinumString, goldString, silverString, copperString;
	local platinum = floor(money / COPPER_PER_PLATINUM);
	local gold = floor((money - (platinum * COPPER_PER_PLATINUM)) / COPPER_PER_GOLD);
	local silver = floor((money - (platinum * COPPER_PER_PLATINUM + gold * COPPER_PER_GOLD)) / COPPER_PER_SILVER);
	local copper = mod(money, COPPER_PER_SILVER);
	...
	if ( ENABLE_COLORBLIND_MODE == "1" ) then
		platinumString = platinum..PLATINUM_AMOUNT_SYMBOL;
		goldString = gold..GOLD_AMOUNT_SYMBOL;
		silverString = silver..SILVER_AMOUNT_SYMBOL;
		copperString = copper..COPPER_AMOUNT_SYMBOL;
	else
		platinumString = format(PLATINUM_AMOUNT_TEXTURE, platinum, 0, 0);
