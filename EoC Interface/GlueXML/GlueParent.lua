...
CharModelFogInfo["TUSKARR"] = { r=0.85, g=0.88, b=1.0, far=500 };
...
CharModelFogInfo["GOBLIN"] = { r=0.8, g=0.65, b=0.73, far=222 };
CharModelFogInfo["WORGEN"] = { r=0.8, g=0.65, b=0.73, far=222 };
...
CharModelGlowInfo["TUSKARR"] = 0.15;
...
GlueAmbienceTracks["ENGINEER"] = "GlueScreenIntro";
...
--		ACCOUNT_MSG_NUM_AVAILABLE = arg1;
		...
		--[[ACCOUNT_MESSAGE_BODY_NO_READ_URL = "http://www.chaoticunited.com/wow/accountmessaging/getMessageBody.php";
		ACCOUNT_MESSAGE_BODY_URL = nil; -- not sure what this is for
		ACCOUNT_MESSAGE_BUTTON_READ = "Mark as read";
		ACCOUNT_MESSAGE_HEADERS_URL = "http://www.chaoticunited.com/wow/accountmessaging/getMessageHeaders.php";
		ACCOUNT_MESSAGE_READ_URL = "http://www.chaoticunited.com/wow/accountmessaging/markRead.php";]]
...
function SetBackgroundModel(model, name)
	...
	if ( name == "Tuskarr" or name == "TUSKARR" ) then 
		if ( model == CharacterCreate ) then
			SetCharCustomizeBackground("Interface\\Glues\\Models\\UI_Dwarf\\UI_Dwarf.m2");
		else
			SetCharSelectBackground("Interface\\Glues\\Models\\UI_Dwarf\\UI_Dwarf.m2");
		end
	elseif (name == "Goblin" or name == "GOBLIN") then
		if ( model == CharacterCreate ) then
			SetCharCustomizeBackground("Interface\\Glues\\Models\\UI_Orc\\UI_Orc.m2");
		else
			SetCharSelectBackground("Interface\\Glues\\Models\\UI_Orc\\UI_Orc.m2");
		end
	end
	if name == "DeathKnight" then
		model:SetSequence(0);
		model:SetCamera(0);
		local fogInfo = CharModelFogInfo[nameupper];
		if ( fogInfo ) then
			model:SetFogColor(20, 20, 20); -- red green black
			model:SetFogNear(20);
			model:SetFogFar(fogInfo.far);
		else
			model:ClearFog();
		...
		local glowInfo = CharModelGlowInfo[nameupper];
		if ( glowInfo ) then
			model:SetGlow(0.1);
		else
			model:SetGlow(0.3);
		...
		local LightValues = RaceLights[nameupper];
		if(LightValues) then 
			for index, Array in pairs (LightValues) do
				if (Array[1]==1) then	-- is this light enabled?
					for j, f in pairs ({model.AddCharacterLight, model.AddLight, model.AddPetLight }) do
	...
	elseif name == "Goblin" then
		...
		local fogInfo = CharModelFogInfo[nameupper];
		if ( fogInfo ) then
			model:SetFogColor(200, 200, 200); -- red green black
			model:SetFogNear(100);
			model:SetFogFar(fogInfo.far);
		else
			model:ClearFog();
		end
